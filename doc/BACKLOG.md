...
| 読み込み完了サマリを保持（リロード後も表示） | request | UI仕様 / 4.2 操作 / 10.3 複数ファイル読み込み | ファイル読み込みで「読み込み完了: ...」が表示され、リロード後も直前のサマリが表示される。新規読み込みで最新内容に更新される。クリア操作でサマリも消える。 | localStorageでの永続化が前提。対象候補: `src/main.ts` / `src/ui/paneMessages.ts`。テスト候補: サマリ文字列生成＋storage読み書きのユニットテスト。 |
| ファイル一覧のカード表示（視認性改善） | request | UI仕様 / 4.2 操作 / 10.3 複数ファイル読み込み | 読み込んだファイルがカードUIで一覧表示される。左右ペインの区別が付く。長いファイル名は省略表示し、hover等で全文が分かる。ライト/ダークで破綻しない。 | 対象候補: `src/ui/template.ts` / `src/style.css`。テスト候補: カード構造/クラス/ラベルのDOM確認（`src/ui/template.test.ts`）。 |
| ファイルカードクリックで該当ファイル先頭へジャンプ | request | UI仕様 / 4.2 操作 / 10.3 複数ファイル読み込み | ファイル一覧の項目クリックで、そのファイルの先頭行へジャンプする。スクロール連動ON/OFFに依存せず破綻しない（OFF時は片側のみ移動）。 | Monacoの `revealLine`/`setPosition`/`focus` を使用。連結表示の開始行番号を保持する必要あり。対象候補: `leftSegments/rightSegments` 管理＋一覧クリックハンドラ。テスト候補: 「ファイル→開始行」解決ロジックのユニットテスト＋クリックで呼ばれる関数の引数検証。 |
