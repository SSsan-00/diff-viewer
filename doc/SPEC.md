...
- エディタ上の通常編集では Monaco が通知する変更範囲を使い、アンカー行より前の行挿入・行削除に合わせて対象側の行番号を追跡する
  - アンカー行内の改行、アンカー行の削除、前後行との結合など、元の論理行を一意に追跡できない構造変更では推測しない
  ...
  - `stale` へ移った後に反対側の本文が変更されている場合は、元側を Undo しても反対側の対応位置を一意に証明できないため、自動復帰せず `stale` を維持する
- 外部ファイルの再読み込みまたは文字コード変更による再デコードでは、同じファイル内だけで対応行を探索する
  - ファイル本文が同一なら同じ file-local 行を維持する
  - 本文が変わった場合は、変更前後の両方で行内容が完全一致かつ一意な場合だけ移動先として採用する
  - 行内容の変更・削除・重複、対象ファイルの消失などで一意に決められない場合は `stale`（要確認）へ移す
