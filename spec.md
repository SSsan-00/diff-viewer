...
- LocalStorage による状態保存・復元の章を追加。
- アンカーパネルの折りたたみUIを仕様化し、変更ブロックの折りたたみと区別。
...
- `アンカーパネル折りたたみ` トグル（変更ブロックの折りたたみとは別のUI）
- アンカーパネル
  - アンカー一覧（クリックでジャンプ）
  - 削除ボタン
  - 無効アンカーの警告表示
...
- アンカーパネルは折りたたみ可能とし、変更ブロックの折りたたみとは独立して制御する
...
- アンカーパネルの折りたたみUIとは無関係（別の機能として扱う）
...
- クリア操作は保存データも削除し、次回起動時に復元されない状態にする
...
- アンカーパネルの折りたたみができ、変更ブロックの折りたたみと独立して動作する
...
- リロード後に前回の状態が復元される（テキスト、アンカー、トグル状態など）
...
- `STRUCTURE.md` を新規作成し、主要ディレクトリと役割を一覧化する
- 構成変更が入る場合は同じPR/コミットで `STRUCTURE.md` を更新する
