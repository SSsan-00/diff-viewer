...
| エディタ上部のスコープ表示（関数/タグ等）を非表示にする | request | UI仕様 / エディタ表示 | エディタ上部に固定表示される関数名/タグ/ブロック等のバー（sticky scroll / scope header相当）が表示されない。差分・アンカー・折りたたみ等の既存機能はデグレしない。`build:single` でも非表示。 | Monaco optionの `stickyScroll` / `stickyScroll.enabled` などを無効化する方針。`src/main.ts` のエディタ生成箇所が対象。テストはエディタ options に無効化設定が含まれることを固定。 |
