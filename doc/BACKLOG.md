...
| --- | --- | --- | --- | --- |
| 対応行判定のブラッシュアップ（インデント差のコメント/キーワード） | bug | 5.2 行レベル差分 | インデント差のみのコメント行・`break;`・`else`・`{` が対応行として揃い、行内差分で空白差が表示される。 | aligned-but-not-equal の扱い維持 |
| ダークテーマ時の視認性向上（行内差分/オートアンカー） | bug | UI仕様 / 4.2 操作 | ダークテーマで行内差分のハイライトとオートアンカー項目が読める配色になる（hover/selected含む）。 | テーマトークン整理が前提 |
...
| 対応行判定のブラッシュアップ（SQL日付整形の意図一致） | request | 5.2 行レベル差分 | `$sql .= \", to_char(date, 'yyyy/mm/dd')\";` と `sql += \", FORMAT(date, 'yyyy/MM/dd')\";` が replace として揃い、行内差分が出る。 | 文字列リテラル一致の強化が前提 |
| 「差分なしの箇所を折りたたみ」表記の変更 | request | 4.2 操作 / UI仕様 | トグル文言を「差分のみ表示」に変更し、機能の意味と一致する。 | 既存トグル挙動維持 |
| 折りたたみトグルの削除（Alt+Z は維持） | request | 4.2 操作 / UI仕様 | 折りたたみトグルを削除し、Alt+Z の折りたたみ操作は残る。UIにはショートカット案内を追加する。 | ショートカット説明の更新 |
