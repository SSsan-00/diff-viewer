...
- 保存なし画面では保存ボタンを表示しない。
- 保存なし画面でも、ブラウザが読み取り用 File System Access ハンドルを提供できる場合は、単一ファイル再読み込みのために読み取りハンドルを保持する。
...
- 保存対象は **単一ファイルを読み込んだペイン** のみとする。
- 複数ファイルを1ペインに連結している場合は保存できない。
- 書き込み用ハンドルを取得できた場合のみ保存できる。
  - ファイル選択ボタン経由で開いた単一ファイル
  - ブラウザが dropped file の writable handle を提供する場合の単一ファイル drag&drop
- 読み取り用ハンドルは IndexedDB に保存し、同じワークスペース/ペインの単一ファイル情報と一致する場合は再読み込み対象として復元できる。
- 保存あり画面では、復元したハンドルが書き込み可能な場合のみ保存対象として扱う。
- 貼り付けなど、元ファイルへの書き込みハンドルを持たない状態では保存できない。

#### 10.5.3 外部変更の再読み込み

- `再読み込み` ボタンは `保存` と `ソースコピー` の間に配置する。
- `再読み込み` は、読み取りハンドルを持つ単一ファイルペインだけで有効になる。
