...
12. **保存あり / 保存なし画面**を URL パラメータで切り替えできる
...
- 既定は **保存なし画面** とする。
- `?save=on` / `?writeback=on` / `?mode=save` または `#save=on` で開いた場合は **保存あり画面** とする。
- `?save=off` / `?writeback=off` / `?mode=no-save` または `#save=off` でも明示的に **保存なし画面** として起動できる。
- 保存なし画面では保存ボタンを表示しない。
...
- 既定は保存なし画面とし、`?save=on` / `?writeback=on` / `?mode=save` または `#save=on` で保存あり画面として起動できる
