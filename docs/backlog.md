...
| 配布物に `http` / `github` 系文字列が残る | build | 3.2 配布物に「特定文字列」を残さない / 13.3 リリース手順 | `dist/index.html` / `dist/index.min.html` に `http://` / `https://` / `github` / `api` / `github.com` 等が **一切含まれない**（`<script>` / `<style>` 内も含める方針を明記）。 |
