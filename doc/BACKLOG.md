...
| --- | --- | --- | --- | --- |
| 折り返しトグルを画面ヘッダへ移動（両ペイン同時） | request | 4.2 操作 / 4.1 レイアウト | 画面ヘッダのスクロール連動の右に配置し、ペインヘッダ内の折り返しは削除。Alt+Z で切替。切替後はレイアウト確定後に再計算され、対応行がズレない。 | レイアウト確定後再計算の共通化が望ましい |
...
| アンカー折りたたみ時も高さを2件分確保してスクロール | request | 4.2 操作 / 8. アンカー行仕様 | 折りたたみ時もアンカー表示は残り、高さは固定（2件分）。overflow-y でスクロールできる。 | なし |
| シンタックスハイライト機能の実装 | request | 10.1 UI仕様 / 4.2 操作 | PHP/C#/HTML/JS など主要言語で色分けが有効。拡張子または言語選択で切替され、file:// 直開きでも動作する。 | Monaco の言語設定と一致させる |
| コメント箇所のグレーアウト | request | UI仕様 / 4.2 操作 | コメントが通常テキストより弱い色で表示され、ライト/ダーク両方で判読可能。テーマ切替で即時反映される。 | シンタックスハイライトの前提あり |
...
| 項目 | 種別 | doc/SPEC.md 反映先候補 | 受け入れ条件のヒント | 依存関係 |
| --- | --- | --- | --- | --- |
| 文字コード選択UIの要否見直し（調査→実装） | request | 10.4.2 文字コード選択の扱い / 10.1 UI仕様 | 自動判定の限界/失敗ケースを整理し、UIを残すか削除する結論をドキュメント化。結論に沿って UI/テスト/仕様を更新。 | 調査結果のドキュメント化が先 |
| 対応行の精度改善（PHP/C# 比較強化） | request | 5.2 行レベル差分 | インデント差で一致が崩れない。`StringBuilder.AppendLine` と `$sql .=` のような意図一致が揃う。誤マッチは抑止。 | 既存のアンカー/ユニーク行優先を維持 |
| Blob 要否調査（リリースビルド） | build | 13.3 リリース手順 / 12.1 配布物 | Blob を使う理由と代替案を結論付きで整理し、使わずに済む場合は方針を定める。機能デグレの有無を明記する。 | verify:dist の方針に影響があれば明記 |
| Blob を使わない構成への移行 | build | 13.3 リリース手順 / 12.1 配布物 | Blob を使わずに同等機能が維持される（読み込み/保存/出力等）。デグレなしをテストで確認。 | Blob 要否調査の結論が先 |
...
| 配布物に `http` / `github` 系文字列が残る | build | 3.2 配布物に「特定文字列」を残さない / 13.3 リリース手順 | `dist/index.html` / `dist/index.min.html` に禁止文字列が含まれない。monaco の該当文字列をパッチで削除し、`build:single` 後の `verify:dist` が Green。 |
