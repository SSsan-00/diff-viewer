...
| --- | --- | --- | --- |
| 折り返しON時に対応行がズレる | bug | 5.2 行レベル差分 / 4.2 操作 | 折り返しONでも対応行（equal/replace）が左右で同じYに揃う。論理行の実描画高を測り、Alt+Z・幅変更・検索UIのレイアウト変更後も不足高を view zone で補正する。 |
| ヘッダの「再計算」ボタン撤去と「レポート出力（HTML）」追加 | request | 4.2 操作 / 7. 再計算仕様 | 再計算ボタンは表示されず、自動再計算フローは維持される。レポート出力は表示どおりの左右2列（仮想行/ギャップ行/ファイル境界ラベル含む）を HTML テーブルでダウンロードでき、Excel へ貼り付けても行が揃う。 |
...
| 先頭の空白を無視トグル | request | 4.2 操作 / 5.2 行レベル差分 | ヘッダの「先頭の空白を無視」トグルで、各行の先頭の spaces/tabs 差分を無視できる。行内空白・末尾空白は対象外で、トグルOFF時は従来挙動のまま。 |
| 各ペインの「ソースをコピー」機能（表示どおり/仮想行含む） | request | 4.2 操作 / 10.3 複数ファイル読み込み | 左右ペインのコピーで、diff整列の仮想行（ギャップ行）と複数ファイル境界ラベル/余白を含む表示列を `\n` 区切りでコピーできる。Excel貼り付け時に表示行数と揃う。 |
...
| PHPタグ付き `if` と Razor `@if` の対応行判定 | request | 5.2 行レベル差分 | 名前・共通比較演算子・リテラルが一致する単純条件だけを replace で揃え、Razor の単独 `{` は insert に残す。空白/タブ揺れと左右反転に対応し、`@while` / `@:if` / 代入 / 呼び出し / 複合条件は誤対応しない。 |
...
| ワークスペース機能（タイトルUI化） | request | UI仕様 / 4.2 操作 | タイトルクリック/Alt+NでワークスペースUIを開閉。最大10件、名前は25文字以内、空文字不可。追加/削除/リネーム/ドラッグ並び替えができ、最後の1件は削除不可。タイトル表示は選択中のワークスペース名のみで、切替時に追従する。ワークスペース一覧の selected スタイルはアクティブな1件のみ。パス登録はワークスペース単位で保存し、旧キーは移行される。左右ペインのテキスト/ファイル一覧/segments/選択/カーソル/スクロール/アンカーはワークスペース単位で保持され、切替時に保存/復元される。切替時は差分を再計算し、読み込みサマリはクリアする。状態は localStorage に保存される。 |
...
| パス登録UIの表示整理（展開時のみ/縦積み/ドラッグ並び替え） | request | UI仕様 / 4.2 操作 | 展開時のみ一覧を表示。登録済みパスは縦積みブロックで表示し、ドラッグ&ドロップで並び替え。左右ペイン別に最大10件まで保存し、読み込み時に10件へ補正してUIの縦スクロールを発生させない。×ボタンは削除。文字入力で入力欄に即フォーカス。↑/↓で選択・Enterでコピー。Deleteで削除。Ctrl+P/Cmd+Pで展開トグル。最大10件がスクロール無しで表示される。コピー成功時はUIが自動で閉じる。 |
...
| 複数ファイル読み込み時の末尾行番号ズレ | bug | 10.3 複数ファイル読み込み / 行番号 | 末尾の行番号が Monaco のモデル行数と一致し、最終ファイルの末尾改行は保持される（余計な空行を追加しない）。 |
| 編集後に file-local 行番号がズレる | bug | 10.3 複数ファイル読み込み / 行番号 | 読み込み後に編集で行数が変わっても file-local の行番号が正しく更新される。 |
