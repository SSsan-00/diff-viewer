# 差分ビュー作成 — 仕様書 v1.2（実装準拠 + 配布要件強化 + 読み込み/アンカー拡張）
...
10. **左右をクリア**できる  
11. **配布物（dist/index.html）はHTMLファイル単体で動く**（利用者はビルド不要・オフラインOK）
...
- ビルド後に `dist/index.html` を対象に **禁止文字列検査（ban words）** を行う
...
- `dist/index.html` に SourceMap 参照（`sourceMappingURL`）を含めない
...
- 最上部に **ツールバー**
- ツールバー直下に **アンカーパネル**
- 各カラムの上部にファイル読み込みUI（文字コード選択 + ドロップ + ファイル選択）
...
4. 同点で判定が付かない場合は、ペインの優先設定を使って決める
   - ペインが `Shift_JIS` 指定なら Shift_JIS を優先
   - ペインが `EUC-JP` 指定なら EUC-JP を優先
   - ペインが `自動` の場合は Shift_JIS を優先（日本語ファイルでの実用性優先）
...
#### 11.1.2 配布物（dist/index.html）受け入れ条件（追加 / 最重要）

- `dist/index.html` は **単一ファイル**で動作する（追加の `.js` / `.css` / `.map` を必要としない）
- `dist/index.html` に **禁止文字列**（3.2）が含まれない
- `dist/index.html` に SourceMap 参照や inline sourcemap が含まれない（3.3）
...
- **配布物検査テスト**: ビルド後の `dist/index.html` に禁止文字列/SourceMapが含まれないこと（置換なし）
- **E2E は後回し**（まず Vitest + Testing Library で十分）

### 11.3 最初に書くべきテスト（MVPの最短ルート）

1. 行差分：同一、挿入、削除、置換の最低ケース  
2. スクロール連動：ON で追従する、OFF で追従しない  
3. 再計算：編集→ボタン→差分更新される  
4. 配布物検査：禁止文字列/SourceMapが混入していない  
...
3. `dist/index.html` が生成されていること
4. `dist/index.html` を `file://` 直開きで主要機能が動くこと
5. **禁止文字列検査（3.2）に合格**していること
6. **SourceMap混入が無い（3.3）**こと
7. **検査回避のための置換を行っていない**こと
