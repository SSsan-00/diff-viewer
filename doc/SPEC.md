...
- 保存なし画面では保存ボタンを表示せず、ファイル選択時も通常の `<input type="file">` 経由で読み込む。
- 保存なし画面では File System Access API による書き込み権限を要求しない。
- 保存あり画面では各ペインヘッダーに `保存` と `再読み込み` ボタンを表示する。
...
- 書き込み用ハンドルは IndexedDB に保存し、同じワークスペース/ペインの単一ファイル情報と一致する場合は復元できる。
- 貼り付けなど、元ファイルへの書き込みハンドルを持たない状態では保存できない。

#### 10.5.3 外部変更の再読み込み

- 保存あり画面では `保存` と `ソースコピー` の間に `再読み込み` ボタンを配置する。
- `再読み込み` は保存対象と同じく、ハンドルを持つ単一ファイルペインだけで有効になる。
...
- 保存あり画面では、復元可能な単一ファイルハンドルを保持し、`再読み込み` で外部変更を差分表示へ反映できる
