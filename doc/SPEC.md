...
- `折り返し` トグル（画面ヘッダで左右同時に切替。Alt+Z でも切替可能。切替後は **レイアウト確定後に再計算** してズレない）
- Ctrl/Cmd+F で、フォーカス中のペインに Monaco の検索UIを開く（フォーカス不明時は最後にフォーカスされたペインを優先）
- ☀️/🌙 トグルでライト/ダークテーマを切り替える（ヘッダートグルと同一UI、即時反映）
- ダークテーマではアンカー一覧の hover/selected とファイル境界のファイル名表示が十分に判別できる配色を維持する
- アンカーパネル
  - アンカー一覧（クリックでジャンプ）
  - 削除ボタン
  - 無効アンカーの警告表示
...
- 折り返しON時でも **論理行の行高を固定** し、対応行の縦位置は常に「論理行の先頭」を基準に揃える（行全体の背景は維持し、再計算タイミングの統一でズレを最小化する）
