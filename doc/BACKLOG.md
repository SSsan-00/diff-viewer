...
| ワークスペース機能（タイトルUI化） | request | UI仕様 / 4.2 操作 | タイトルクリック/Alt+NでワークスペースUIを開閉。最大10件、名前は25文字以内、空文字不可。追加/削除/リネーム/ドラッグ並び替えができ、最後の1件は削除不可。タイトル表示は選択中のワークスペース名のみで、切替時に追従する。ワークスペース一覧の selected スタイルはアクティブな1件のみ。パス登録はワークスペース単位で保存し、旧キーは移行される。左右ペインのテキストとアンカーはワークスペース単位で保持され、切替時に保存/復元される。切替時は差分を再計算し、ファイルカード/読み込みサマリ/セグメントをリセットする。状態は localStorage に保存される。 |
