...
- 文字コード（エンコーディング）をペインごとに選択できる
  ...
  - `自動` は **UTF-8 BOM の有無 + UTF-8 の厳密デコード**のみ
  - `自動` で UTF-8 として正しくデコードできない場合は「文字コードを選択してください」を表示する
...
#### 11.1.1 配布物（dist/index.html）受け入れ条件（追加 / 最重要）
