...
| --- | --- | --- | --- | --- |
| Ctrl+F でペイン内検索ができない | bug | 4.2 操作 | Ctrl/Cmd+F でフォーカス中の Monaco の検索UIが開く。ブラウザ検索は抑止される。フォーカス不明時は最後にフォーカスしたペインに寄せる。 | なし |
| ダークテーマの視認性改善（アンカー一覧/ファイル名表示） | bug | UI仕様 / 4.2 操作 | ダークテーマ時にアンカー一覧の hover/selected が読める配色になり、ペイン内のファイル名表示が見える。テーマ切替で即時反映される。 | なし |
...
| 2ファイル読み込み時の順序調整（cshtml.cs → cshtml） | request | 10.3 複数ファイル読み込み | File.name に `*.cshtml` と `*.cshtml.cs` がある場合は `cshtml.cs` を先に読み込む。順序がテストで固定される。 | なし |
