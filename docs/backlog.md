...
| --- | --- | --- | --- | --- |
| アンカー解除時の丸マーカーが残る | bug | 8. アンカー行仕様 / 4.2 操作 | 片側だけアンカーを設定した状態で同じ行を再クリックすると丸マーカーが消える。両側アンカーの解除時は左右両方の丸が消える。 | なし |
| Ctrl+F をフォーカス中のペイン検索にする | request | 4.2 操作 | 左/右エディタのどちらにフォーカスがあるかで検索対象が切り替わる。ブラウザ標準の検索ではなく Monaco の検索UIが開く。 | なし |
| 文字コード変更時に再解釈（再デコード）する | request | 10.4.2 文字コード選択の扱い | 読み込み後に文字コードを変更すると、同じ入力（元バイト列）を指定エンコーディングで再デコードして表示が更新される。再読み込みを強制しない。 | 元バイト列の保持 |
...
| 項目 | 種別 | spec.md 反映先候補 | 受け入れ条件のヒント | 依存関係 |
| --- | --- | --- | --- | --- |
| 対応行検出の追加ケース（PHP/C# 定数） | request | 5.2 行レベル差分 | `define('FOO','foo');` と `public readonly string FOO = 'foo';` が同じブロック/同一表示行に揃う。近似語の誤マッチは避ける。 | なし |
